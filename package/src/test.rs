...
fn test_pants_2_32_dev2_using_python_3_14(scie_pants_scie: &Path) {
    integration_test!("Verifying we can run Pants 2.32.0dev2+, which uses Python 3.14");
    // Pants 2.32+ is built on macOS 14 (arm64), and only truly supports that version.
...
fn test_pants_2_32_dev1_using_python_3_11(scie_pants_scie: &Path) {
    ...
    if is_macos_thats_too_old(14) {
        log!(
            Color::Yellow,
            "Pants 2.32.0dev1 cannot run on this version of macOS => skipping"
        );
        return;
    }

    let pants_version = "2.32.0.dev1";
    let output = execute(
        Command::new(scie_pants_scie)
            .env("PANTS_VERSION", pants_version)
            .arg("-V")
...
fn test_pants_from_sources_mode(
    ...
    softlink(pants_2_31_0_dev2_clone_dir, &pants_dir).unwrap();
