...
pub(crate) fn run_integration_tests(
    ...
    if matches!(
        *CURRENT_PLATFORM,
        ...
        let pants_2_24_0_dev3_clone_dir = clone_dir.join("pants-2.24.0.dev3");
        let venv_dir = dev_cache_dir.join("venvs");
        let pants_2_24_0_dev3_venv_dir = venv_dir.join("pants-2.24.0.dev3");

        test_pants_source_mode(
            scie_pants_scie,
            &clone_dir,
            &pants_2_24_0_dev3_clone_dir,
            &venv_dir,
            &pants_2_24_0_dev3_venv_dir,
        );
        ...
        test_delegate_pants_in_pants_repo(scie_pants_scie, &pants_2_24_0_dev3_clone_dir);
        test_use_pants_release_in_pants_repo(scie_pants_scie, &pants_2_24_0_dev3_clone_dir);
...
fn test_pants_source_mode(
    scie_pants_scie: &Path,
    clone_dir: &Path,
    pants_2_24_0_dev3_clone_dir: &Path,
    venv_dir: &Path,
    pants_2_24_0_dev3_venv_dir: &Path,
) {
    ...
    // invalid, we start getting errors like `${pants_2_24_0_dev3_venv_dir}/.../bin/python: No such file
    ...
    if !pants_2_24_0_dev3_clone_dir.exists() || !pants_2_24_0_dev3_venv_dir.exists() {
        ...
        // N.B.: The release_2.24.0.dev3 tag has sha 6f8e16e0a2b78d9b3feeb860b38782b92abd1848 and we
        // must pass a full sha to use the shallow fetch trick.
        const PANTS_2_24_0_DEV3_SHA: &str = "6f8e16e0a2b78d9b3feeb860b38782b92abd1848";
        execute(
            Command::new("git")
                .args([
                    "fetch",
        ...
        execute(
            Command::new("git")
                .args(["reset", "--hard", PANTS_2_24_0_DEV3_SHA])
                .current_dir(clone_root_tmp.path()),
        )
        .unwrap();
        write_file(
...
+echo >&2 "The ${SCIE_PANTS_TEST_MODE:-Pants 2.24.0.dev3 clone} is working."
+
 function venv_dir() {
...
"#,
        ...
        rename(&clone_root_tmp.into_path(), pants_2_24_0_dev3_clone_dir).unwrap();
        ensure_directory(venv_dir, true).unwrap();
        rename(&venv_root_tmp.into_path(), pants_2_24_0_dev3_venv_dir).unwrap();
    }

    assert_stderr_output(
        Command::new(scie_pants_scie)
            .arg("-V")
            .env("PANTS_SOURCE", pants_2_24_0_dev3_clone_dir)
            .env("SCIE_PANTS_TEST_MODE", "PANTS_SOURCE mode")
            .env("PANTS_VENV_DIR_PREFIX", pants_2_24_0_dev3_venv_dir),
        vec![
...
fn test_pants_from_sources_mode(
    ...
    softlink(pants_2_24_0_dev3_clone_dir, &pants_dir).unwrap();
...
fn test_delegate_pants_in_pants_repo(
    scie_pants_scie: &Path,
    pants_2_24_0_dev3_clone_dir: &PathBuf,
) {
    ...
    assert_stderr_output(
        Command::new(scie_pants_scie)
            ...
            .current_dir(pants_2_24_0_dev3_clone_dir),
        vec![
